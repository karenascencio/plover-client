...
export default function NavBarPatient (props) {
  ...
  return (
    <div className='z-50  '>
      ...
      <div className='sm:sticky top-0  flex flex-row sm:flex-col justify-between sm:justify-start items-center px-080 sm:pt-10 h-20 w-100vw sm:h-100% sm:w-30vw sm:max-w-sm lg:max-w-18rem xl:max-w-26rem  bg-plover-blue'>
        ...
        <ul className=' mb-10 mt-20 hidden sm:flex sm:flex-col sm:justify-around sm:items-center h-80 '>
          {
                        forms.map((item, key) => {
                          return (
                            ...
                            ><button onClick={(() => handleOption(item.form))} className='text-white text-lg '>{item.title}</button>
                            </motion.li>
                          )
                        })
                    }

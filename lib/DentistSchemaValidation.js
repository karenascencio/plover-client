...
export const loggedPasswordSchema = Yup.object({
  password: Yup.string()
    .required('Para cambiar tu contraseña debes poner tu contraseña actual'),
  newPassword: Yup.string()
    .required('Se debe ingresar una contraseña nueva')
    .matches(
      /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/,
      'La contraseña debe contener al menos una letra mayúscula, una minúscula, un digito y un caracter especial(?@/.. etc)'
    ),
  comparePassword: Yup.string()
    .required('Debes reingresar la contraseña nueva')
